...
import { getSanitizedEnv } from "./spawn-utils.js";
...
export class LspClient {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        this.process.stderr?.on("data", () => {
          /* LSP servers log to stderr; ignored */
        });
    ...
    while (this.buffer.length > 0) {
      if (this.contentLength === -1) {
        ...
        if (this.contentLength > MAX_MESSAGE_SIZE || this.contentLength < 0) {
          console.warn(
            `[pi-lens] Dropping oversized message (${this.contentLength} bytes, max ${MAX_MESSAGE_SIZE})`,
          );
          this.buffer = "";
  ...
  request<T = unknown>(
    ...
    return new Promise<T>((resolve, reject) => {
      ...
      this.server.pendingRequests.set(id, {
